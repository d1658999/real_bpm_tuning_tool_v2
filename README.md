...
2. Assign every active port in the middle panel's **Port configuration** table. Its four columns are `File`, `Port`, `Mode`, and `Port configuration`; the final column automatically changes to the measured-part picker, per-port L/C range controls, connection destination, signal assignment, or open/short summary required by the selected mode. Connections must be reciprocal, and signal names must be unique and consecutive from `s1` (two to four total). A file that is not yet part of the circuit may remain in the project when every one of its ports is `open`; it is ignored by cascade and optimization.
...
5. Set **BOM samples/type** globally, then set the inclusive nominal **L** and/or **C** range inside each tunable port row. `inductor` shows only its nH range, `capacitor` shows only its pF range, and the two combined modes show both ranges on separate lines. Each port defaults to the complete measured catalog and may use a different window. The optimizer filters that port's real catalog before choosing its evenly spaced samples. Reversed, non-positive, non-finite, or empty measured-part ranges produce an actionable warning naming the file and port. The sample default is 2; increasing it covers more real parts but multiplies the Cartesian search at every tunable port. The GUI warns before searches above 100,000,000 combinations or an estimated 10-minute native sweep.
...
Apply one command-line fallback window to every tunable port that does not already have an individual range:

```powershell
bpm-tuner --candidates 7 --inductor-range 0.1 10 --capacitor-range 0.1 100
```

Saved JSON configurations store `inductor_min_nh`, `inductor_max_nh`, `capacitor_min_pf`, and `capacitor_max_pf` in each entry under `networks[].ports[]`. Older project-wide range fields and CLI range options remain supported as fallbacks; configurations without either form use the complete measured catalogs.
